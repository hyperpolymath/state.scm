...
=== questions-for-maintainer (OPTIONAL)

Categorized questions requiring human input, preventing AI from making assumptions.

[source,scheme]
----
(questions-for-maintainer
  ((architecture
     ("Should analytics integration use Erlang Port or WASM compilation?"
      "Is the multi-database adapter pattern intentional?"))
   (priorities
     ("Is v0.2 higher priority than v0.3?"))
   (integration
     ("Which SSO provider should be primary?"))
   (deployment
     ("Kubernetes or bare metal for production?"))
   (testing
     ("Is 80% coverage sufficient for release?"))
   (community
     ("Accept external contributors for core modules?"))))
----
...
|`priorities`
|Ordering and sequencing of work

|`integration`
|External service/system choices
...
This section captures open questions that need human input, ensuring AI assistants
do not make assumptions about decisions that require maintainer judgment.

=== long-term-roadmap (OPTIONAL)

Extended roadmap for future planning beyond MVP, including vision and research.

[source,scheme]
----
(long-term-roadmap
  ((post-v1
     ((v1.1 ((name . "Enhanced Analytics")
             (items . ("Dashboard" "Metrics export"))))
      (v1.2 ((name . "Advanced Security")
             (items . ("Audit logging" "RBAC"))))))
   (vision-items
     ("Universal project manager integration"
      "AI-assisted code review"
      "Formal verification integration"))
   (research-tracks
     ((formal-verification
        ((goal . "Prove correctness of core algorithms")
         (status . "investigating")))
      (performance
        ((goal . "Sub-millisecond query latency")
         (status . "benchmarking")))))))
----

==== Long-Term Roadmap Subfields
...
|`post-v1`
|Object[]
|Planned releases after initial major version

|`vision-items`
|String[]
|Aspirational features without timeline

|`research-tracks`
|Object[]
|Exploratory work with goals and status
|===

=== state-summary (OPTIONAL)
...
Implementations SHOULD provide standard helper functions for querying state:

[source,scheme]
----
;; Get overall completion percentage
(define (get-completion-percentage state)
  (assoc-ref (assoc-ref state 'state-summary) 'overall-completion))

;; Get all blockers across projects
(define (get-blockers state)
  (let ((projects (assoc-ref state 'projects)))
    (apply append
      (map (lambda (p) (assoc-ref p 'blockers))
           projects))))

;; Get specific milestone information
(define (get-milestone state milestone-name)
  (let ((roadmap (assoc-ref state 'long-term-roadmap)))
    (assoc milestone-name (assoc-ref roadmap 'post-v1))))

;; Check if project is blocked
(define (project-blocked? state project-name)
  (let* ((projects (assoc-ref state 'projects))
         (project (find (lambda (p)
                          (equal? (assoc-ref p 'name) project-name))
                        projects)))
    (and project
         (equal? (assoc-ref project 'status) "blocked"))))
----

These functions enable programmatic access to state data and support
relational queries via minikanren integration.
